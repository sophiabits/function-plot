...
function linearY (chart, datum, range, nSamples) {
  // range is calculated using xScale in evaluate.js, so we need to recalculate it here
  var yRange = datum.range || [-Infinity, Infinity]
  var scale = chart.meta.yScale
  var yEndpoints = [
    Math.max(scale.domain()[0], yRange[0]),
    Math.min(scale.domain()[1], yRange[1])
  ]
  var yCoords = utils.space(chart, yEndpoints, nSamples)

  var samples = []
  for (var i = 0; i < yCoords.length; i += 1) {
    var y = yCoords[i]
    var x = evaluate(datum, 'fn', { y: y })
    samples.push([x, y])
  }
  return [samples]
}
